...
rule_guild_list = []
rule_group_list = []
for per_server in plugin_config.mc_qq_server_dict.values():
    ...
    if group_list := per_server.group_list:
        for per_group in group_list:
            rule_group_list.append(per_group.group_id)


async def msg_rule(event: Union[GroupMessageEvent, GuildMessageEvent, MessageCreateEvent]) -> bool:
    """Rule 消息规则"""
