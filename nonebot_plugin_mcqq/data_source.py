...
async def _ws_handler(websocket: WebSocket):
    ...
    ori_server_name = websocket.request.headers.get("x-self-name")

    if not ori_server_name:
        # 服务器名为空
        logger.error(f"[MC_QQ]丨未获取到该服务器的名称，连接断开")
        await websocket.close(1008, "[MC_QQ]丨未获取到该服务器的名称，连接断开")
        return

    server_name = ori_server_name.encode("utf-8").decode("unicode_escape")
    ...
    CLIENTS[server_name] = Client(
        server_name=server_name, websocket=websocket, rcon=rcon_client
    )
